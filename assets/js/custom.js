$(window).load(function(){
  ...
  let content_div = JSON.parse(sessionStorage.getItem("sesCars"));
  if (content_div==null) return;
  let total = 0;
  for(let i = 0; i < content_div.length; i++){
    name = content_div[i].brand;
    price = "$" + content_div[i].price;
    img = content_div[i].id;
    quantity = content_div[i].qty;
    total += quantity;
    if (checkQty(img, name, 0)==0)
    {
